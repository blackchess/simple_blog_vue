...
export default {
  ...
  methods: {
    getCommentList(number) {
      ...
      this.$http.get('getComment.php', { params: this.queryInfo }).then(res => {
    ...
    async deleteComment(commentId, comment) {
      ...
      const { data: res } = await this.$http.get('deleteEditComment.php', { params: { id: commentId, username: this.commentForm.username } })
    ...
    editComment(){
        ...
        const { data: res } = await this.$http.post('deleteEditComment.php', { commentForm: this.commentForm })
        if (res.result.status != 1) {
          return this.$message.error(res.result.mgs)
        }
