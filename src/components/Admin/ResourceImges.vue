...
export default {
  ...
  methods: {
    ...
    async handleDelete(url) {
      ...
      // 发送请求: 执行删除请求
      this.$http.get('deleteFile.php?fileName='+fileName+'&userName='+userName).then((res) => {
        const { data } = res;
        if (data.status === 1) {
    ...
    getSourceImage() {
      this.$http.get('getSourceImage.php', { params: { start: this.start, size: this.size } }).then((res) => {
