...
<script>
import {fetchList} from '@/api/service.article.js'
...
export default {
  ...
  methods: {
    ...
    async getComment() {
      ...
      const { data: res } = await this.$http.get('addComment.php', { params: { blog_id } })
    ...
    addComment() {
      this.$refs.commentFormRef.validate(async (valid) => {
        ...
        const { data: res } = await this.$http.post('addComment.php', { commentForm: this.commentForm })
        if (res.result.status != 1) {
    ...
    //   const blog_id = this.$route.query.blog_id;
    //   const { data: res } = await this.$http.get('commentReplay.php', { params: { blog_id } })
    //   if (res.result.status != 1) {
    //     return
    //   }
    ...
    addReplay() {
      this.$refs.commentFormRef.validate(async (valid) => {
        ...
        this.commentForm.blog_id = this.BlogContainerData[0];
        this.commentForm.comment = this.objUsername + this.commentForm.comment
        const { data: res } = await this.$http.post('commentReplay.php', { commentForm: this.commentForm })
        if (res.result.status != 1) {
